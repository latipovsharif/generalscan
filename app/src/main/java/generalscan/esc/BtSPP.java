...
public class BtSPP {
	...
	public static boolean OpenPrinter(String BDAddr)
	{
    	...
    	{
    		ErrorMessage="No Bluetooth Printer is Selected!";
    		return false;
    	}
    	myBluetoothAdapter = BluetoothAdapter.getDefaultAdapter();
    	if(myBluetoothAdapter==null)
    	{
    		ErrorMessage="Bluetooth Printer Error!";
    		return false;
    	}
    	myDevice = myBluetoothAdapter.getRemoteDevice(BDAddr);
    	if(myDevice==null)
    	{
    		ErrorMessage="Error for Read Bluetooth Printer";
    		return false;
    	}
	...
	public static boolean SPPOpen(BluetoothAdapter bluetoothAdapter, BluetoothDevice btDevice)
	{
		...
		{
			ErrorMessage = "Bluetooth is not open!";
	        return false;
		}
...
*/		
		...
		{
			ErrorMessage = e2.getLocalizedMessage();//"Can not Connect Bluetooth Printer";
			mySocket = null;
			return false;
		}
...
/*		
		...
		{
			ErrorMessage = "Error for Send Bluetooth data";
			return false;
		}
		return true;
*/		
	...
	public static boolean SPPReadTimeout(byte[] Data,int DataLen,int Timeout)
	{
		...
		for(i=0;i<(Timeout/50);i++)
		{
			try 
			{
				if(myInStream.available()>=DataLen)
				{
					...
					{
						ErrorMessage = "Read BLuetooth Printer data error!";
						return false;
					}
				}
			} 
		...
		ErrorMessage = "Time expired for Read Bluetooth data!";
		return false;
	}
}
